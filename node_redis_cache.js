...
var redis_lru_cache = {
	...
	initialize:function(params, done){
		...
		try
		{
		    ...
		    this.client = this.redis.createClient(this.params.port, this.params.ip, this.params.options);
	...
	getAllInternal:function(collectionKey, cacheConfig, ttl, done)
	{
		...
		this.client.smembers(key, function(e, resp){
			...
			if (!e)
			{
				...
				if (resp != null && resp.length > 0)
				{
					...
					this.async.forEach(resp, function(respItem, callback) { //The second argument (callback) is the "task callback" for a specific messageId
					...
					/*
					this.async.until(function(){ return fetchedCount == resp.length}.bind(this), 
					function(){
						getSingleItem(resp[fetchedCount], function(e, item){
	...
	putInternal:function(collectionKey, key, item, cacheConfig, ttl, done)
	{
		...
		else
		{
			...
			try
			{
				...
				this.client.set(setParameters, function(e, response){
					...
					if (!e)
					{
						...
						if (collectionKey != null)
						{
							...
							this.client.sadd(saddParameters, function(sadd_e, sadd_response){
	...
	{
		this.client.expire(key, ttl, done);
	}
}

module.exports = redis_lru_cache;